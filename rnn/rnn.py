...
for i in range(1, 20):
    if(predicted_stock_price[i - 1] < predicted_stock_price[i]):
        p_buy.append(real_stock_price[i])
        p_buy_x.append(i)
    else:
        p_sell.append(real_stock_price[i])
        p_sell_x.append(i)

p_buy = np.array(p_buy)
...
plt.scatter(p_buy_x, p_buy, color = 'green', label = 'Buy')
plt.scatter(p_sell_x, p_sell, color = 'red', label = 'Sell')
